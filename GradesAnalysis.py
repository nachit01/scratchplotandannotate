...
class Grade:
    ...
    def __init__(self, rootdatapath=r"evo_images_CSV", gradesdtatfoldername="data_grades",
                 mergedfilename=r"merged_grades_data.csv"):
    ...
    def pltogradesbyside(self, sides=None):
        ...
        for side in sides:
            ...
            # plot
            ax = dfmergeddatagarde.plot(kind="scatter", x=gradecolumn, y=valuecolumn, grid=True, legend=True, c='blue',
                                        s=50)
    ...
    def addinteractivemismatch(slef, df, ax, colimei, colvalues, colgrade, colmangrade, side):
        # Add interactive cursors
        cursor = mplcursors.cursor(ax, hover=True)
        ...
        def create_on_add(df, colimei, colgrade, colvalues, colmangrade):
            def on_add(sel):
                imei = df[colimei].iloc[sel.target.index]
                mangrade = df[colmangrade].iloc[sel.target.index]
                sel.annotation.set(text=f'imei:{imei}\n'
                                        f'mangrade:{mangrade}\n'
                ...
                def open_image(event):
                    imagename = f"{AllPhonesSidesCVData.IMAGESFILENAMESBYSIDE[side][:-4]}_{mangrade}_{imei}.jpg"
                    imagepath = Path(".").cwd() / r"evo_images_CSV" / r"all_annotated_images"

                    annotation_box = sel.annotation.get_bbox_patch()
                    # Get the bounding box in display (pixel) coordinates
                    bbox = annotation_box.get_window_extent()
                    # Check if the click is inside the annotation's bounding box
                    if bbox.contains(event.x, event.y):
                        #check file exists. Grade txt EVO file doesn't give imei-0...
                        if os.path.exists(imagepath / imagename):
                            os.startfile(imagepath / imagename)  # Opens the image in the default Windows viewer
                        else:
                            #get the name that exists in all_annotated_image with imei-X
                            for i in range(10):
                                imagename = f"{imagename[:-4]} - {i}.jpg"
                                if os.path.exists(imagepath / imagename):
                                    os.startfile(imagepath / imagename)  # Opens the image in the default Windows viewer
                                    break
                    sel.artist.figure.canvas.mpl_disconnect(cid)

                # Connect the click event to the canvas
                cid = sel.artist.figure.canvas.mpl_connect(
                    "button_press_event", open_image
                )
